...
class MarineBiomesEnv(gym.Env):
    ...
    Actions:
        Type: Discrete(2)
        Num    Action
        0      Maintain the biomass.
        1      Increase the biomass.
    ...
    def step(self, action):
        ...
        biomass_variation = action * self.relative_acceleration * \
        ...
        tempreture = ((self.min_tempreture+self.max_tempreture)/2) + \
        ...
        done = bool(biomass >= self.goal_biomass)
        # done = bool(self.timestep >= 50)
        reward = None
    ...
    def reset(self):
        self.timestep = 0
        self.state = np.array(
            [self.np_random.uniform(low=95, high=100), 15])
