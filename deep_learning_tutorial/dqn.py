...
class QNetwork:
    def __init__(self, learning_rate=0.01, state_size=4, action_size=2, hidden_size=10):
        ...
        self.optimizer = "adam"  # 誤差を減らす学習方法はAdam
...
mainQN = QNetwork(hidden_size=hidden_size,
                  learning_rate=learning_rate)     # メインのQネットワーク
targetQN = QNetwork(hidden_size=hidden_size,
                    learning_rate=learning_rate)   # 価値を計算するQネットワーク
...
for episode in range(num_episodes):  # 試行数分繰り返す
    ...
    state, reward, done, _ = env.step(
        env.action_space.sample())  # 1step目は適当な行動をとる
    ...
    for t in range(max_number_of_steps + 1):  # 1試行のループ
        ...
        next_state, reward, done, info = env.step(
            action)   # 行動a_tの実行による、s_{t+1}, _R{t}を計算する
        # list型のstateを、1行4列の行列に変換
