...
class FNAgent:
    def __init__(self, epsilon, actions) -> None:
    ...
    @classmethod
    def load(cls, env: gym.Env, model_path, epsilon=10**(-4)):
    ...
    def initialize(self, experiences):
        raise NotImplementedError("You have to implement initializing method.")
    ...
    def play(self, env: gym.Env, episode_count=5, render=True):
...
class Trainer:
    def __init__(self, buffer_size=1024, batch_size=32, gamma=0.9, report_interval=10, log_dir="") -> None:
    ...
    def train_loop(self, env: gym.Env, agent: FNAgent, episode=200, initial_count=-1, render=False, observe_interval=0):
        ...
        for i in range(episode):
            ...
            while not done:
                if render:
                    env.render()
                if self.training and observe_interval > 0 and (self.training_count == 1 or self.training_count % observe_interval == 0):
                    frames.append(s)
                    a = agent.policy(s)
                    n_state, reward, done, info = env.step(a)
    ...
    def episode_end(self, episode, step_count, agent: FNAgent):
        pass
    ...
    def get_recent(self, count):
        ...
        return [self.experiences[i] for i in range(recent)]
